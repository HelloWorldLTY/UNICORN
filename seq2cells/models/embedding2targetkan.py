...
class Embedding2Target(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.hparams.model_type in ["linear", "provided"]:
            ...
            modules.append(FastKANLayer(self.hparams.emb_dim, self.hparams.bottleneck_dim))
#             modules.append(nn.BatchNorm1d(self.hparams.bottleneck_dim, affine=True))
#             modules.append(nn.TransformerEncoder(
#             nn.TransformerEncoderLayer(d_model=self.hparams.bottleneck_dim, nhead=8, dropout=self.hparams.dropout_prob, activation=nn.GELU(),
#                                        batch_first=True),
#             num_layers=1
#         ))
...
#                 modules.append(nn.ReLU())
            ...
            modules.append(
                FastKANLayer(self.hparams.bottleneck_dim, self.hparams.target_dim)
            )
