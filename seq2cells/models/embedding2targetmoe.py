...
class Embedding2Target(pl.LightningModule):
    ...
    def __init__(
        ...
        if self.hparams.model_type in ["linear", "provided"]:
            ...
            modules.append(FMoETransformerMLP(
                  8, 
                  d_model = self.hparams.bottleneck_dim,
                  d_hidden = self.hparams.bottleneck_dim,
                  top_k = 2
                ))
...
#             modules.append(nn.Softplus())
        ...
        assert self.hparams.optimizer in ["AdamW", "SGD", "RMSprop", "Adabelief"], (
...
#             y_hat_sp = F.softmax(y_hat, dim=0)
#             y_sp = (y>0)*1
#             loss = F.mse_loss(y_hat, y) + F.cross_entropy(y_hat_sp, y_sp.float())
...
#             y_hat_sp = F.softmax(y_hat, dim=0)
#             y_sp = (y>0)*1
#             val_loss = F.mse_loss(y_hat, y) + F.cross_entropy(y_hat_sp, y_sp.float())
            l1 = self.pearson_loss(y_hat, y)
            l2 = F.poisson_nll_loss(y_hat, y, log_input=False)
            l3 = F.mse_loss(y_hat, y)
            val_loss = l1 + l2 + l3  
